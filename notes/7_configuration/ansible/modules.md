...
#### Хорошая практика использовать `ansible.builtin.` для обращения к основным модулям 
...
- ###### lineinfile - Ищет линию в файле и заменяет, либо добавляет, если ее не было.

```yml
- name: Добавить DNS server в resolv.conf
  lineinfile:
    path: /etc/resolv.conf
    line: 'nameserver 10.1.20.2'
```
