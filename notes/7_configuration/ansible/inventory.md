...
ansible_sudo_pass: zxc
```



Можно обратиться к любому хосту из инвентаря и забрать любую его персональную переменную (например, `pod_cidr` или `ansible_host`) из любой точки плейбука несколькими способами:

1) Цикл `loop: {{ groups['workers'] }}`, на item будет первое значение из списка inventory.
2) Прямое обращение по имени хоста (Жесткая привязка)

```yaml
- name: Показать сеть подов для node-0
  ansible.builtin.debug:
    msg: "Сеть подов: {{ hostvars['node-0']['pod_cidr'] }}"
```