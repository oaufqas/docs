...
# Разбор цепочек из Docker и Kubernetes

## Таблица filter (фильтрация пакетов)


 ###### Chain INPUT (пакеты, идущие к самому хосту)

|Правило|Объяснение|
|---|---|
|`KUBE-PROXY-FIREWALL`|Защита для load balancer сервисов. Проверяет новые соединения (ctstate NEW)|
|`KUBE-NODEPORTS`|Обработка NodePort сервисов (порты 30000-32767). Также health check сервисов|
|`KUBE-EXTERNAL-SERVICES`|Маршрутизация трафика к сервисам с ExternalIP или LoadBalancer|
|`KUBE-FIREWALL`|Блокирует локальные соединения (из 127.0.0.0/8) к сервисам, которые не должны их получать|

###### Chain FORWARD (транзитные пакеты через хост)

|Правило|Объяснение|
|---|---|
|`KUBE-PROXY-FIREWALL`|Та же защита, что и в INPUT|
|`KUBE-FORWARD`|**Главная цепочка K8s для форварда**|
|`KUBE-SERVICES`|Маршрутизация к сервисам (для случаев, когда хост проксирует трафик)|
|`KUBE-EXTERNAL-SERVICES`|Внешние сервисы|
|`CNI-FORWARD`|**Правила CNI плагина** (обычно Calico или Flannel)|
|`DOCKER-USER`|Пользовательские правила Docker|
|`DOCKER-FORWARD`|Docker forwarding|

###### **Chain KUBE-FORWARD** (разбор)

|Правило|Объяснение|
|---|---|
|`DROP ... ctstate INVALID`|Отбрасывает пакеты с невалидным состоянием (защита от атак)|
|`ACCEPT ... mark match 0x4000/0x4000`|Пропускает пакеты, помеченные K8s (маркер 0x4000 = "это наш трафик")|
|`ACCEPT ... ctstate RELATED,ESTABLISHED`|Пропускает уже установленные соединения|

###### **Chain CNI-FORWARD** (разбор)

|Правило|Объяснение|
|---|---|
|`CNI-ADMIN`|Административные правила CNI|
|`ACCEPT ... 10.244.0.193 ctstate RELATED,ESTABLISHED`|Пропускает уже установленные соединения к конкретному поду (IP 10.244.0.193)|
|`ACCEPT ... 10.244.0.193`|Пропускает трафик **от** этого пода|

**Что такое 10.244.0.193?** Это IP-адрес пода в сети CNI (обычно Flannel использует сеть 10.244.0.0/16). Каждое правило — отдельный под в кластере.

# Таблица nat (трансляция адресов)

Это самая важная таблица для Kubernetes. Именно здесь происходит магия ClusterIP и NodePort.

###### Chain PREROUTING (до маршрутизации)

|Правило|Объяснение|
|---|---|
|`KUBE-SERVICES`|**Самое важное правило!** Перехватывает трафик, идущий к ClusterIP сервисов, и перенаправляет на KUBE-SVC-* цепочки|
|`DOCKER_OUTPUT`|Docker: перенаправляет запросы к host.minikube.internal|
|`DOCKER`|Docker: обрабатывает локальные адреса (ADDRTYPE match dst-type LOCAL)|

###### Chain OUTPUT (исходящие пакеты)

|Правило|Объяснение|
|---|---|
|`KUBE-SERVICES`|Перехватывает исходящие запросы к ClusterIP (например, под обращается к другому сервису)|
|`DOCKER_OUTPUT`|Docker|
|`DOCKER`|Docker для не-loopback адресов|

###### Chain POSTROUTING (после маршрутизации)

|Правило|Объяснение|
|---|---|
|`KUBE-POSTROUTING`|Маскарадинг (SNAT) для трафика, идущего от подов наружу|
|`MASQUERADE 172.17.0.0/16`|Docker bridge network (классический docker0)|
|`CNI-*`|**Отдельная цепочка для каждого пода!** Занимается маскарадингом трафика от конкретного пода|

---

#### Расшифровка цепочек KUBE-SVC и KUBE-SEP

Это сердце service mesh в iptables.

 - **KUBE-SVC-xxx** (Service) - Представляет собой **ClusterIP сервис**. Содержит правила балансировки нагрузки на поды.
-  **KUBE-SEP-xxx** (Service EndPoint) - Представляет собой **конкретный под (endpoint)** сервиса.

### Пример из вывода (сервис `dp-svc:nginx`):

**KUBE-SVC-2GQI7DNT3UQ2XTIE** (сервис с IP 10.110.57.108 на порту 80)

|Правило|Объяснение|
|---|---|
|`KUBE-MARK-MASQ ... !10.244.0.0/16`|Если источник не из сети подов — поставить маркер 0x4000 (нужно будет SNAT)|
|`KUBE-SEP-7QKYKHQKF53F45PL`|33.3% трафика → под 10.244.0.198:80|
|`KUBE-SEP-LODTPWVG4RLZLCI5`|50% от оставшегося → под 10.244.0.200:80|
|`KUBE-SEP-3WQO5N3RO4JMIQMJ`|Оставшийся трафик → под 10.244.0.208:80|

**Как работает probability:**

- Первое правило: `probability 0.333...` → 1/3 трафика
- Второе правило: `probability 0.500...` → 1/2 от оставшихся 2/3 = еще 1/3
- Третье правило: остаток → еще 1/3


**KUBE-SEP-7QKYKHQKF53F45PL** (эндпоинт для пода 10.244.0.198)

|Правило|Объяснение|
|---|---|
|`KUBE-MARK-MASQ`|Поставить маркер на пакет (для последующего SNAT)|
|`DNAT ... to:10.244.0.198:80`|**Заменить IP назначения** с ClusterIP на реальный IP пода|

---

# Docker цепочки (кратко)

|Цепочка|Назначение|
|---|---|
|`DOCKER`|Правила для контейнеров, работающих в host network|
|`DOCKER-FORWARD`|Форвардинг между контейнерами|
|`DOCKER-USER`|Пользовательские правила (применяются до DOCKER-FORWARD)|
|`DOCKER-CT`|Отслеживание состояния (conntrack) для Docker|
|`DOCKER-BRIDGE`|Связывает правила Docker bridge сети|

**DOCKER_OUTPUT** и **DOCKER_POSTROUTING** в таблице nat обеспечивают работу встроенного DNS в Docker (`127.0.0.11`).

---

## 🎯 CNI цепочки (почему их так много?)

Каждый под в кластере получает **собственную цепочку** в таблице nat:

```text
CNI-45a2f215a61e11e663ccff6d
CNI-9954ba9520cfb62ca64fc2df
CNI-fb537de73100a606a7ba34fd
```


**Зачем?** CNI плагин (Flannel в Minikube) создает отдельную цепочку для каждого пода, чтобы:

1. Применять маскарадинг (SNAT) для исходящего трафика пода
2. Управлять политиками сети (network policies)
3. Обеспечивать изоляцию между подами


__Что внутри CNI-_ цепочки?_*

|Правило|Объяснение|
|---|---|
|`ACCEPT ... 10.244.0.0/16`|Пропускать трафик внутри сети подов (не маскарадить)|
|`MASQUERADE ... !base-address.mcast.net/4`|Маскарадить трафик во внешние сети (не в мультикаст)|

---

## Маркер 0x4000 (KUBE-MARK-MASQ)

Это **ключевая концепция** Kubernetes в iptables:

|Где встречается|Значение|
|---|---|
|`KUBE-MARK-MASQ`|Ставит маркер 0x4000 на пакет|
|`KUBE-POSTROUTING`|Если маркер есть → делаем SNAT|
|`KUBE-FORWARD`|Если маркер есть → пропускаем пакет|
|`KUBE-SVC-*`|Ставит маркер на пакеты, пришедшие НЕ из сети подов|

**Логика:**

1. Пакет приходит извне (не из 10.244.0.0/16) → K8S ставит маркер 0x4000
2. При прохождении через FORWARD пакет с маркером пропускается
3. При выходе пакет с маркером маскарадится (SNAT)