...
Важный нюанс: Образ состоит из слоев. Если вы измените только одну строчку в коде, Docker при сборке не будет пересобирать весь образ целиком, а обновит только нужный слой. Это делает Docker очень быстрым. 

**Вся файловая структура докера представляет собой большое количество переиспользуемых слоев**
...
Разница между `ENTRYPOINT ["python3", "-u", "main.py"]` и `ENTRYPOINT python3 -u main.py`  заключается в том, что при втором варианте, в образе создается дополнительный процесс shell, который будет ловить системные вызовы, если они предусмотрены в приложении. во втором варианте код запускается напрямую с пайтона, без промежуточного шелла. 