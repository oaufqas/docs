...
#### 2. Есть две изолированные сети /25 - 192.168.1.0 (gw: 192.168.1.1), 192.168.1.128 (gw: 192.168.1.129). Есть два сервера со следующими таблицами маршрутизации
...
Второй сервер будет думать, что первый находится в его сети, и будет отправлять пакеты напрямую, а не шлюзу, поэтому они будут теряться
...
192.168.1.146
routes
192.168.1.0/25 192.168.1.129 # Изменено
```
