...
Необходимо настроить podAntiAffinity. Данное указание определяет, что для определенных подов следует использовать их размещание на разных нодах. NodeSelector - параметры серверов, чтобы например база открывалась на ssd, фронтенд на hdd.
...
По умолчанию при запуске одного контейнера в одном поде запускается еще *pause* контейнер для сохранения адреса сети. Итого, в одном поде может быть запущено *n+1* контейнеров. Также есть sidecar контейнеры, например vault injector, или какой нибудь мониторинг за приложением.
...
**Deployment** — это стандартный, родной (native) объект самого **Kubernetes**.

**DeploymentConfig (DC)** — это специализированный, устаревший объект от компании Red Hat, созданный специально для платформы **OpenShift**. 
Умеет «следить» за вашим внутренним реестром образов (ImageStream) . Как только CI/CD сборщик собрал новый образ тега `:latest`, `DeploymentConfig` это видит и **автоматически** запускает обновление подов на проде.
...
Liveness проба - это постоянные проверки жизни контейнера, когда он работает. Например каждые 30 сек.
Readness и Startup - это проверки готовности контейнера после запуска, когда можно слать на него трафик.
